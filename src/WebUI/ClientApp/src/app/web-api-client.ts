...
export interface ITimeTrackingClient {
    ...
    get(requestedDate: Date | undefined): Observable<TimeTrackingDto[]>;
}
...
export class TimeTrackingClient implements ITimeTrackingClient {
    ...
    get(requestedDate: Date | undefined): Observable<TimeTrackingDto[]> {
        ...
        if (requestedDate === null)
            throw new Error("The parameter 'requestedDate' cannot be null.");
        else if (requestedDate !== undefined)
            url_ += "RequestedDate=" + encodeURIComponent(requestedDate ? "" + requestedDate.toISOString() : "") + "&";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<TimeTrackingDto[]>;
                }
            } else
                return _observableThrow(response_) as any as Observable<TimeTrackingDto[]>;
        }));
    }

    protected processGet(response: HttpResponseBase): Observable<TimeTrackingDto[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                ...
                for (let item of resultData200)
                    result200!.push(TimeTrackingDto.fromJS(item));
            }
            else {
                result200 = <any>null;
            }
